...
EXPECTED_IDS = {"SMK-001", "SMK-005", "STR-001", "RO-001", "EDT-002"}
...
class TestCaseFiles(unittest.TestCase):
    ...
    def test_five_cases_on_disk(self):
        self.assertEqual(len(self.files), 5, [p.name for p in self.files])
    ...
    def test_smk_001_shape(self):
        ...
        # SMK-001 stays the strict whole-edit-format probe (no trailing newline).
        self.assertIs(case["validator"]["args"]["trailing_newline"], False)

    def test_smk_005_shape(self):
        case = loader.load_case(CASES_DIR / "smoke" / "SMK-005.json")
        self.assertEqual(case["mode"], "write")
        self.assertEqual(case["validator"]["type"], "exact_text")
        self.assertEqual(case["allowed_paths"], ["out/banner.txt"])
        # SMK-005 is the trailing-newline-tolerant sibling of SMK-001.
        self.assertIs(case["validator"]["args"]["trailing_newline"], True)
        self.assertIn("frontend-quirk-trailing-newline", case["tags"])
